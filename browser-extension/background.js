...
class WatSearchBackground {
    ...
    async handleCourseData(data, sendResponse) {
        try {
            console.log(
                "WatSearch Background: Sending data to WatSearch:",
                data,
            )
            ...
            console.error(
                "WatSearch Background: Error handling course data:",
                error,
            )
        ...
        try {
            ...
            for (const port of alternativePorts) {
                try {
                    ...
                    console.log(
                        `WatSearch Background: Port ${port} not available`,
                    )
                }
            }
    ...
    async handleBatchDownload(urls, selectedTerms, sendResponse) {
        try {
            ...
            if (
                ...
                (typeof urls === "object" && !urls.coursesByTerm && !urls.urls)
            ) {
                this.isProcessing = false
                sendResponse({
                    success: false,
            ...
            for (let i = 0; i < urlsToProcess.length; i++) {
                ...
                if (this.shouldCancel) {
                    console.log(
                        "WatSearch Background: Processing cancelled by user",
                    )
                    chrome.runtime.sendMessage({
                ...
                try {
                    ...
                    if (this.shouldCancel) {
                        if (tab && tab.id) {
                            ...
                            this.currentProcessingTabs =
                                this.currentProcessingTabs.filter(
                                    (id) => id !== tab.id,
                                )
                        }
                        break
                    ...
                    const apiResponse = await this.sendOutlineToWatSearch(
                        htmlResponse.html,
                    )
                    ...
                    console.error(
                        `WatSearch Background: Failed to process ${url}:`,
                        error,
                    )
                ...
                if (
                    completedCount === urlsToProcess.length &&
                    !this.shouldCancel
                ) {
                    chrome.runtime.sendMessage({
                        type: "DOWNLOAD_COMPLETE",
            ...
            console.error(
                "WatSearch Background: Error in batch processing:",
                error,
            )
            this.isProcessing = false
            this.currentProcessingTabs = []
    ...
    async sendOutlineToWatSearch(html) {
        try {
            ...
            const response = await fetch(
                `${this.watSearchUrl}/api/upload-outline`,
                {
                    method: "POST",
                    body: formData,
                },
            ...
            console.error(
                "WatSearch Background: Failed to send outline to API:",
                error,
            )
            throw error
        }
