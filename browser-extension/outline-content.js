...
class OutlineContent {
  ...
  extractCourseUrls() {
    ...
    const termSections = document.querySelectorAll('.border.rounded, div.border');
    ...
    if (Object.keys(coursesByTerm).length === 0) {
      const viewLinks = document.querySelectorAll('a[href*="/viewer/view/"]');
      const urls = [];
      ...
      console.log(`WatSearch Outline: Extracted ${urls.length} course outline URLs (no term grouping found)`);
      return { urls, coursesByTerm: {} };
    }

    console.log(`WatSearch Outline: Extracted courses from ${Object.keys(coursesByTerm).length} terms`);
  ...
  extractCourseInfo() {
    const courseCode = document.querySelector('.outline-courses')?.textContent?.trim();
    const term = document.querySelector('.outline-term')?.textContent?.trim();
    const title = document.querySelector('.outline-title-full')?.textContent?.trim();
