...
class EnhancedWatSearchExtractor {
  ...
  async extractLEARNData() {
    ...
    for (const link of contentLinks.slice(0, this.maxPages)) {
      try {
        ...
        console.error(`WatSearch: Error following link ${link.url}:`, error);
      }
    }
  ...
  extractAssignmentsFromPage() {
    ...
    console.log(`WatSearch: Found ${allPageLinks.length} total links on page`);
    ...
    console.log(`WatSearch: Final result - ${assignments.length} assignments`);
    return assignments;
  }
  ...
  extractAnnouncementsFromPage() {
    ...
    console.log(`WatSearch: Extracted ${announcements.length} announcements`);
  ...
  async extractData() {
    console.log(`WatSearch: Enhanced extraction from ${this.currentSite}...`);
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          ...
          console.log("WatSearch: Enhanced data sent successfully:", response);
        }
      }
    );
  }
