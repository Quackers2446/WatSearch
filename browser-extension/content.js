...
class WatSearchDataExtractor {
  ...
  extractLEARNData() {
    ...
    courseElements.forEach((element, index) => {
      ...
      const courseCode = this.extractText(element, ".course-code, .course-id");
      ...
      if (courseName || courseCode) {
        courses.push({
          ...
          instructor: this.extractText(element, ".instructor, .professor"),
    ...
    assignmentElements.forEach((element, index) => {
      ...
      const dueDate = this.extractText(element, ".due-date, .deadline, .date");
    ...
    announcementElements.forEach((element, index) => {
      ...
      const content = this.extractText(element, ".content, .message, .body");
  ...
  extractQuestData() {
    ...
    scheduleRows.forEach((row, index) => {
      ...
      const location = this.extractText(row, ".location, .room, .building");
    ...
    courseElements.forEach((element, index) => {
      const courseCode = this.extractText(element, ".course-code, .subject");
  ...
  extractPiazzaData() {
    ...
    postElements.forEach((element, index) => {
      const title = this.extractText(element, ".post-title, .title, h3, h4");
      const content = this.extractText(element, ".content, .body, .message");
