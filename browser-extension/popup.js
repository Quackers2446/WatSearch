...
class WatSearchPopup {
  ...
  initializeElements() {
    ...
    this.downloadOutlinesBtn = document.getElementById("downloadOutlinesBtn");
    this.cancelProcessingBtn = document.getElementById("cancelProcessingBtn");
  ...
  setupEventListeners() {
    ...
    this.downloadOutlinesBtn.addEventListener("click", () => this.downloadAllOutlines());
  ...
  async detectCurrentSite() {
    try {
      ...
      if (hostname.includes("learn.uwaterloo.ca")) {
        ...
        this.showSiteInfo("Outline.uwaterloo.ca", "Course outline viewer");
  ...
  async loadAvailableTerms() {
    ...
    try {
      ...
      if (extractResponse && extractResponse.success && extractResponse.coursesByTerm) {
        this.coursesByTerm = extractResponse.coursesByTerm;
        this.availableTerms = Object.keys(extractResponse.coursesByTerm).sort((a, b) => {
  ...
  renderTermSelection() {
    ...
    if (this.availableTerms.length === 0) {
      this.termCheckboxes.innerHTML = '<div style="font-size: 11px; color: #666; padding: 8px;">No terms found. Make sure you\'re on the listings page.</div>';
      return;
    }

    // Default to selecting the first (most recent) term
    const defaultSelected = this.availableTerms.length > 0 ? [this.availableTerms[0]] : [];

    this.availableTerms.forEach((term, index) => {
      const isChecked = index === 0; // Default to first term
      const checkbox = document.createElement('label');
      checkbox.style.cssText = 'display: flex; align-items: center; gap: 6px; font-size: 12px; cursor: pointer; padding: 4px 0;';
      ...
      const labelText = document.createTextNode(`${term} (${courseCount} courses)`);
  ...
  getSelectedTerms() {
    const checkboxes = this.termCheckboxes.querySelectorAll('input[type="checkbox"]:checked');
  ...
  async downloadAllOutlines() {
    ...
    try {
      ...
      if (!extractResponse || !extractResponse.success) {
        throw new Error(
          extractResponse?.error || "Failed to extract course outline URLs"
        );
      }
