...
export default function Dashboard({ courses, deadlines }: DashboardProps) {
    ...
    const thisWeekDeadlines = deadlines.filter(
        (d) => isThisWeek(d.dueDate) && !isToday(d.dueDate),
    )
    const nextWeekDeadlines = deadlines.filter((d) =>
        isSameWeek(d.dueDate, addWeeks(new Date(), 1), { weekStartsOn: 1 }),
    )
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
                ...
                <p className="text-gray-600">
                    Your personalized view of courses, deadlines, and academic
                    progress
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="card">
                    ...
                    <div className="space-y-4">
                        {courses.map((course) => {
                            const courseDeadlines = deadlines.filter(
                                (d) => d.courseCode === course.code,
                            )
                            const upcomingCount = courseDeadlines.filter(
                                (d) => d.isUpcoming,
                            ).length
