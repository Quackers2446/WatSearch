...
import { Search, Filter, Calendar, BookOpen, Clock, ChevronDown } from 'lucide-react'
...
export default function SearchInterface({
    ...
    const filteredResults = useMemo(() => {
        ...
        if (filters.type) {
            filtered = filtered.filter(course =>
                course.assessments.some(assessment => assessment.type === filters.type)
            )
        }
        ...
        if (filters.upcoming) {
            filtered = filtered.filter(course =>
                course.assessments.some(assessment =>
                    ...
                    new Date(assessment.dueDate) <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
                )
            )
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-4">Search Academic Resources</h2>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => onFiltersChange({ ...filters, courseCode: e.target.value })}
                            ...
                            <div className="flex items-center space-x-2">
                                <input
                                    ...
                                    onChange={(e) => onFiltersChange({ ...filters, upcoming: e.target.checked })}
                                    className="rounded"
                                />
                        ...
                        {filteredResults.map((course) => (
                            <div key={course.id} className="card">
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <h4 className="text-lg font-semibold text-gray-900">{course.code}</h4>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center text-sm text-gray-600">
                                        ...
                                        <span className="ml-2">{course.schedule.location}</span>
                                    </div>
                                </div>
