...
function batchProcessOutlines() {
  ...
  const files = fs.readdirSync(courseOutlinesDir)
    .filter(file => file.endsWith('.html') && file !== 'Outline.uwaterloo.ca.html')
  ...
  files.forEach((filePath, index) => {
    ...
    try {
      ...
      if (course) {
        ...
        console.log(`  ✓ Parsed: ${course.code} - ${course.name} (${course.term})`);
      } else {
        errors.push({ file: fileName, error: 'Failed to parse course outline' });
  ...
  parsedCourses.forEach(newCourse => {
    const existingIndex = mergedCourses.findIndex(c => 
      c.code === newCourse.code && c.term === newCourse.term
    );
